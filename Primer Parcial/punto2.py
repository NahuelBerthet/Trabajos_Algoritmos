...
especies_unicas = set()
while pila.size() > 0:
    especies_unicas = pila.pop()

print("Cantidad de especies: ", len(especies_unicas))
...
descubridores = set()
while pila.size() > 0:
    descubridores = pila.pop()

print("Cantidad de descubridores: ", len(descubridores))
...
print("Los dinosaurios que empiezan con T son: ", pila_aux)

#Punto d.
pila_peso = []
for dinosaurio in dinosaurios:
    peso_str = dinosaurio["peso"].replace(" kg", "")
    peso = float(peso_str)
    if peso < 275 :
...
print("Dinosaurios con menos de 275 kg")
print(pila_peso["nombre"], pila_peso["peso"])
#Punto e.

for dinosaurio in dinosaurios:
    resultado= Stack()
    if dinosaurio["nombre"].startswith(tuple("A")):
        resultado.push(dinosaurio["nombre"])
