...
class BinaryTree:
    ...
    def search(self, key):
        def __search(root, key):
            if root is not None:
                if root.value == key:
                    # print('lo encontre')
                    return print(root.value,root.other_value)  #Se modifico para el parcial(muestra todos los datos)
                elif key < root.value:
    ...
    def proximity_search(self, search_value):
        def __proximity_search(root, search_value):
            if root is not None:
                ...
                if root.value.startswith(search_value) or search_value in root.value : #Modificado(con que aparezca el caracter lo muestra)
    ...
    def proximity_search_tipo(self, tipo_buscado): #Nueva funcion para buscar por tipo
        def __proximity_search(root, tipo_buscado):
            if root is not None:
                __proximity_search(root.left, tipo_buscado)
                if tipo_buscado in root.other_value['tipos']:
                    print(root.other_value['nombre'])
                __proximity_search(root.right, tipo_buscado)
        print(f"Pokémon de tipo {tipo_buscado}:")
        __proximity_search(self.root, tipo_buscado)
